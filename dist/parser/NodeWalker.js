...
const TextRenderer_1 = require("../renderer/TextRenderer");
const ImageRenderer_1 = require("../renderer/ImageRenderer");
class NodeWalker {
    constructor(context) {
    ...
    async visit(node) {
        ...
        switch (node.type) {
            case 'text':
                await TextRenderer_1.TextRenderer.render(node, this.context);
                break;
            case 'block':
            ...
            case 'break':
                ...
                break;
            case 'image':
                await ImageRenderer_1.ImageRenderer.render(node, this.context);
                break;
        }
            ...
            if (cell.children) {
                ...
                if (cell.styles.textAlign) {
                    cell.children.forEach(c => {
                        if (!c.styles.textAlign)
                            c.styles.textAlign = cell.styles.textAlign;
                    });
                }
                ...
                if (heightUsed > maxHeightUsed)
                    maxHeightUsed = heightUsed;
                // Restore margins
